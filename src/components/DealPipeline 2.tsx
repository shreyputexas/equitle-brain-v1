...
export default function DealPipeline({
  ...
  const handleOpenSelectContacts = async () => {
    ...
    try {
      ...
      const existingContactIds = dealToEdit.people?.map(p => p.id) || [];
  ...
  const handleAddContactsToDeal = async () => {
    if (!editingDealId || selectedContactIds.length === 0) return;

    try {
      setContactsLoading(true);

      // Add each selected contact to the deal
      let successCount = 0;
      let failCount = 0;
      ...
      // Fetch the updated contacts for this deal
      const updatedContacts = await contactsApi.getContacts({ dealId: editingDealId });

      // Update the local deal's people array with the new contacts
      const updatedPeople = updatedContacts.contacts.map((contact: any) => ({
        id: contact.id,
        name: contact.name,
        role: contact.title || contact.role || 'To be updated',
        email: contact.email || '',
      ...
      // Update the state to reflect the new contacts immediately
      // This ensures the dropdown shows the updated contacts right away
      setUpdatedDealContacts(prev => {
        const newMap = new Map(prev);
        newMap.set(editingDealId, updatedPeople);
        return newMap;
      });

      // Show success message
      const message = failCount > 0
        ? `${successCount} contact(s) added successfully, ${failCount} failed`
        : `${successCount} contact(s) added successfully`;
      ...
      // Refresh the deals to update the contacts list in the backend
      ...
      setSnackbarMessage(`Error adding contacts: ${error.message || 'Unknown error'}`);
      setSnackbarOpen(true);
    } finally {
      setContactsLoading(false);
    }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const updatedDeal = {
        ...
        value: isNaN(valueInMillions) ? 0 : valueInMillions * 1000000,
        leadPartner: editFormData.leadPartner,
                ...
                <Box sx={{ 
                  ...
                  {deal.people.map((person) => (
                    ...
                    >
                      ...
                      {/* Relationship Score */}
                      <Box sx={{ mb: 1, p: 1, bgcolor: '#ffffff', borderRadius: 1, border: '1px solid #e5e7eb' }}>
                        ...
                        <LinearProgress
                          ...
                          sx={{
                            ...
                            '& .MuiLinearProgress-bar': {
                              borderRadius: 2,
                              background: 'linear-gradient(90deg, #9ca3af 0%, #000000 100%)'
                            }
                          }}
                        />
                      </Box>
                      ...
                      {/* Summary */}
                      {person.summary && (
                        <Box sx={{ mb: 1, p: 1, bgcolor: '#ffffff', borderRadius: 1, border: '1px solid #e5e7eb', wordBreak: 'break-word' }}>
                          <Typography 
                            variant="caption" 
                            sx={{ 
                              color: '#6b7280',
            ...
            >
              ...
              <DialogActions sx={{ px: 2, py: 1.5, borderTop: '1px solid #e5e7eb' }}>
                ...
                <Button
                  ...
                  disabled={selectedContactIds.length === 0 || contactsLoading}
                  sx={{
                    bgcolor: '#000000',
                    color: 'white',
                    fontFamily: '"Inter", "SF Pro Display", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',
